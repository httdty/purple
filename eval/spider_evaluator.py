...
class EvaluateTool(object):
    ...
    def register_golds(self, dataset, db_path):
        for idx, sample in enumerate(dataset):
            ...
            if db_id not in self.schema_cache:
                self.schema_cache[db_id] = dump_db_json_schema(db=os.path.join(db_path, db_id, f"{db_id}.sqlite"), f=db_id)
    ...
    def evaluate(self, preds):
        ...
        test_suite = compute_test_suite_metric(preds, self.golds, db_dir=None, verbose=self.verbose)

        return {**exact_match, **test_suite}

    def evaluate_one(self, idx, prediction):
        if not self.spider_evaluator or not self.test_suite_evaluator:
            self._init_evaluator()
        reference = self.golds[idx]

        s_score = self.spider_evaluator.evaluate_one(
            reference["db_id"],
            reference["query"],
            prediction
        )
